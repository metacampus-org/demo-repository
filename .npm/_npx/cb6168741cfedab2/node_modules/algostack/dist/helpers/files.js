 ...
 * ==================================================
 */
export function getFileContent(url, responseType = 'text') {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield axios.get(url, { responseType });
...
export function getFileType(url) {
    return __awaiter(this, void 0, void 0, function* () {
        if (isDomainUrl(url) && !isIpfsSubdomain(url))
            return 'text/html';
        const contentType = getContentTypeFromUrl(url);
        if (contentType)
            return contentType;
        try {
            const head = yield axios.head(url, { maxRedirects: 0 });
 ...
 * ==================================================
 */
export function getIpfsCid(url) {
 ...
 * ==================================================
 */
export function getIpfsFromAddress(params, url = 'template-ipfs://{ipfscid:1:raw:reserve:sha2-256}') {
    ...
    if (multicodec === 'dag-pb')
        cidCodecCode = 0x70;
    const addr = decodeAddress(address);
    const mhdigest = digest.create(mfsha2.sha256.code, addr.publicKey);
    const cid = CID.create(Number(cidVersion), cidCodecCode, mhdigest);
 ...
 * ==================================================
 */
export function generateAddressFromIpfs(cidStr) {
