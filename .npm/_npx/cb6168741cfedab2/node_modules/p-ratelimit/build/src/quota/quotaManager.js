...
const dequeue_1 = require("../dequeue");
/** keep track of API invocations, allowing or disallowing them based on our quota */
class QuotaManager {
    constructor(_quota) {
        this._quota = _quota;
        this._activeCount = 0;
        this.history = new dequeue_1.Dequeue();
        if (typeof _quota !== 'object') {
            console.warn('[p-ratelimit QuotaManager] A QuotaManager was created with no quota.');
            this._quota = {};
        }
        if (('interval' in this._quota && !('rate' in this._quota)) ||
            ('rate' in this._quota && !('interval' in this._quota))) {
            const msg = `[p-ratelimit QuotaManager] Invalid Quota: for a rate-limit quota, both ` +
